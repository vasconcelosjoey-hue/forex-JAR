...
export const Roadmap: React.FC<RoadmapProps> = ({ state, addTransaction, deleteTransaction, updateState }) => {
  ...
  return (
    <div className="flex flex-col gap-6 w-full font-mono pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <Card className="lg:col-span-4 relative flex flex-col justify-center bg-[#000] border-[#FF6F00]">
              <div className="flex items-center justify-between mb-2">
                  <span className="text-xs uppercase text-white font-black tracking-widest">Total Aportado</span>
                  <div className="flex items-center gap-2 border border-[#FF6F00] px-2 py-0.5 bg-[#FF6F00]/10">
                      <span className={`text-xs font-black text-[#FF6F00]`}>{progressPercentage.toFixed(2)}%</span>
                  </div>
              </div>
               <div className="text-4xl lg:text-5xl font-black text-white tracking-tighter">
                  <span className="text-lg text-[#FF6F00] mr-2">R$</span>
                  {totalBrlDeposited.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
              </div>
              <div className="mt-4 pt-2 border-t border-dashed border-white/20 text-right text-[10px] font-black text-white/50 uppercase">
                  Meta: R$ 55.000,00
              </div>
          </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {(['JOEY', 'ALEX', 'RUBINHO'] as RoadmapPartner[]).map(partner => (
                <Input
                  ...
                  placeholder="0,00"
                  actionButton={
                    <button onClick={() => handleDeposit(partner)} className="bg-[#111] hover:bg-[#FF6F00] hover:text-black border-l-2 border-white/20 text-neutral-200 h-full px-4 transition-colors flex items-center justify-center rounded-none">
                      <Check size={16} />
                    </button>
                  }
                />
      ...
      <div className="bg-[#000] border-2 border-white/20 p-8 shadow-[4px_4px_0px_0px_white]">
        <div className="relative w-full h-12 flex items-center">
          <div className="absolute top-1/2 left-0 right-0 h-4 bg-[#111] -translate-y-1/2 border border-white/10"></div>
          <div className="absolute top-1/2 left-0 h-4 bg-[#FF6F00] -translate-y-1/2 transition-all duration-1000 border-r-2 border-white" style={{ width: `${progressPercentage}%` }}></div>
            ...
            {CHECKPOINTS.map((cp) => {
               ...
               return (
                <div key={cp.label} className="flex flex-col items-center justify-center relative z-10 w-10">
                    <div className={`w-3.5 h-3.5 border-2 transition-all mb-6 transform rotate-45 ${reached ? (isFinal ? 'bg-white border-white' : 'bg-[#FF6F00] border-white') : 'bg-[#000] border-neutral-700'}`}></div>
                    <span className={`absolute bottom-0 text-[10px] font-black bg-black px-1 border border-white/10 ${reached ? 'text-[#FF6F00]' : 'text-white/30'}`}>{cp.label}</span>
                </div>
               );
            })}
          </div>
         ...
         {(['JOEY', 'ALEX', 'RUBINHO'] as RoadmapPartner[]).map(partner => {
             ...
             return (
                 <div key={partner} className="bg-[#000] border-2 border-white/20 p-4 relative group hover:border-[#FF6F00] transition-colors">
                     <div className="absolute top-0 left-0 w-1.5 h-full bg-[#FF6F00] opacity-0 group-hover:opacity-100 transition-opacity"></div>
                     <h3 className="text-[10px] font-black text-white/50 uppercase tracking-widest mb-2 pl-2 group-hover:text-white transition-colors">{partner}</h3>
      ...
      <Card className="flex flex-col p-0 border-white/20 bg-[#000]">
         ...
         <div className="w-full">
             <table className="w-full text-left border-collapse">
                 <thead className="bg-[#000] text-[10px] uppercase text-white font-black border-b-2 border-white/20">
                     <tr>
                         <th className="py-3 px-6">Data</th>
                         <th className="py-3 px-6">Sócio</th>
                         <th className="py-3 px-6 text-right">Valor R$</th>
                         <th className="py-3 px-4 text-center w-10">...</th>
                     </tr>
                 </thead>
                 <tbody className="text-sm font-black text-white font-mono">
                     {state.transactions
                        .filter(t => t.type === 'DEPOSIT')
                        .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
                        .map(t => (
                         <tr key={t.id} className="border-b border-white/10 hover:bg-white hover:text-black transition-colors group">
                             <td className="py-3 px-6 text-xs text-white/70 group-hover:text-black">{new Date(t.date).toLocaleDateString('pt-BR')}</td>
                             <td className="py-3 px-6 group-hover:text-black">{t.partner}</td>
                             <td className="py-3 px-6 text-right text-[#FF6F00] group-hover:text-black">R$ {t.amountBrl.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                             <td className="py-3 px-4 text-center">
                                 <button onClick={() => deleteTransaction(t.id)} className="text-white/30 hover:text-[#ff4444] transition-colors p-1"><Trash2 size={14} /></button>
                             </td>
                         </tr>
                     ))}
                 </tbody>
